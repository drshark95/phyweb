    ...
    <div className="sticky top-0 z-40 w-full bg-white/70 backdrop-blur border-b border-slate-100">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-lg font-semibold tracking-tight">Spectra.run</span>
        </div>
    ...
    <section className="grid place-items-center py-24">
      <div className="text-center">
        ...
        <p className="mx-auto mt-4 max-w-xl text-slate-600">
          설치 없이 시작하는 과학 수업 웹 키트. 교사는 링크만, 학생은 브라우저만.
        </p>
        <div className="mt-10">
          <button
            onClick={onStart}
  ...
  return (
    <section>
      <header className="mb-6">
        ...
        <p className="text-slate-600">원하는 토픽을 선택하면 즉시 데모 레슨으로 이동합니다.</p>
      </header>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {topics.map((t) => (
          ...
          >
            <div className="text-sm uppercase tracking-wide text-slate-400">Physics</div>
            ...
            <div className="mt-6 inline-flex items-center gap-2 text-indigo-600 font-medium">
              들어가기 <span className="opacity-0 group-hover:opacity-100 transition">→</span>
            </div>
          </button>
        ))}
      </div>
...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="rounded border border-slate-200 bg-slate-50 px-1.5 py-0.5 text-xs">{children}</kbd>
  );
}
    ...
    <div className="prose prose-slate max-w-none">
      {section === "intro" && (
        <>
          <h2>도입 — 빛의 문법 읽기</h2>
          <p>
            네온사인과 수소 방전관의 빛을 분해하면 무지개 띠가 아니라 <strong>선</strong>으로 나타납니다.
            왜 띄엄띄엄일까요?
          </p>
          <ul>
          ...
          <p>
            <em>한 문장:</em> “빛의 간격은 원자의 <strong>에너지 준위</strong>라는 문법을 드러낸다.”
          </p>
        </>
      )}
      ...
      {section === "observe" && (
        <>
          ...
          <p className="text-sm text-slate-500">※ 실제 수치는 장비에 따라 다소 차이</p>
        </>
      )}
      ...
      {section === "model" && (
        <>
          <h2>모델·식 — 전이와 파장</h2>
          <p>
            보어 모형에서 에너지 준위는 <code>E_n = -13.6 Z^2 / n^2\, (eV)</code>로 근사합니다.
            전이 <code>n₂ → n₁</code>에 대해 방출 광자의 에너지는
            <code>ΔE = 13.6 Z^2 (1/n₁² - 1/n₂²)</code> 이고, <code>λ = hc/ΔE</code> 입니다.
          </p>
          ...
          <FormativeQuiz />
            ...
            {section === "wrap" && (
              <>
                <h2>정리 — 한 문장</h2>
                <p>
                  “선의 간격은 원자의 에너지 준위 문법이다. 전이가 클수록 빛은 더 짧은 파장으로 나온다.”
                </p>
                <p className="text-sm text-slate-500">다음: Z가 커지면(He⁺) 문법은 어떻게 달라질까?</p>
              </>
            )}
          </div>
        );
      ...
      function FormativeQuiz() {
        ...
        // Q3: 개념 선택형
        const [q3, setQ3] = React.useState<"shorter" | "same" | "longer" | "">("");
        ...
        const grade = () => {
          ...
          res.push({
            ...
            correct: c2e && c2l,
            value: `${q2e}, ${q2l}`,
            misconception: c2e && !c2l ? "lambda" : !c2e && c2l ? "energy" : (!c2e && !c2l ? "both" : undefined),
          });
          ...
          res.push({
            ...
            correct: c3,
            value: q3,
            misconception: c3 ? undefined : q3 === "longer" ? "inverse_relation" : "undecided",
          });
        ...
        const exportCSV = () => {
          ...
          const rows = attempts
            ...
            .map(
              (a) =>
                `${a.itemId},${a.try},${a.correct ? 1 : 0},"${(a.value || "").replaceAll('"', '""')}",${
                  a.misconception || ""
                }`
            )
            .join("\n");
          ...
          <div className="not-prose">
            <div className="grid gap-6">
                  ...
                  {([1, 2] as const).map((t) => (
                    <button
                      ...
                      className={
                        ...
                        (tryIndex === t ? "bg-slate-900 text-white" : "hover:bg-slate-50")
                      }
                    >
                      {t}차
                    </button>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="text-sm font-medium text-slate-500 mb-2">F1 · 단답</div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="text-sm font-medium text-slate-500 mb-2">F2 · 계산(2칸)</div>
                ...
                <div className="text-slate-400 text-sm mt-1">허용 오차: ΔE±0.05, λ±1.0</div>
              </div>
      
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div className="text-sm font-medium text-slate-500 mb-2">F3 · 개념</div>
                  ...
                  {[
                    ...
                    <label key={opt.id} className="inline-flex items-center gap-2">
                      <input
                        ...
                        name="q3"
                        value={opt.id}
                        checked={q3 === (opt.id as any)}
                        onChange={() => setQ3(opt.id as any)}
                      />
                      <span>{opt.label}</span>
                    </label>
                  ))}
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="text-2xl font-semibold">
                    {pre !== null && post !== null ? `${Math.round((post - pre) * 100)} pp` : "—"}
                  </div>
                </div>
              </div>
    ...
    <footer className="border-t border-slate-100 bg-white/60 backdrop-blur">
      <div className="mx-auto max-w-6xl px-4 py-6 text-sm text-slate-500 flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          <span className="rounded-full bg-slate-100 px-3 py-1">페이드 전환</span>
          <span className="rounded-full bg-slate-100 px-3 py-1">토픽 → 레슨</span>
          <span className="rounded-full bg-slate-100 px-3 py-1">사이드바 네비</span>
        </div>
      </div>
    </footer>
  );
